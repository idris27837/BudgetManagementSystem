...
namespace BudgetManagementSystem.DataAccessLayer.Concrete
{
    ...
    { 
                    ...
                    while(properties.MoveNext())
                    {
                        ...
                        if (isSchemaExt || IsAttributeAuditable(tableName, propertyName))
                        {
                            ...
                            if (!Equals(property.CurrentValue, property.OriginalValue))
                            {
                                ...
                                try
                                {
                                    if (!string.IsNullOrEmpty(propertyName) && propertyName.ToLower().Equals("workproducts"))
                                    {
                                        if (property.OriginalValue != null)
                                        {
                                            var stringData = JsonSerializer.Serialize(property.OriginalValue);
                                            auditLog.OriginalValue = stringData;
                                        }
                                        if (property.CurrentValue != null)
                                        {
                                            var stringData = JsonSerializer.Serialize(property.CurrentValue);
                                            auditLog.NewValue = stringData;
                                        }
                                    }
                                }
                                catch { }
        ...
        {
            bool auditable = true;
            if (_auditableEntity == null) return false;
        ...
        private static void FillAuditableEntityAndAttributes(EntityEntry dbEntry)
        {
            ...
            _auditableEntity = ((CompetencyCoreDbContext)dbEntry.Context).AuditableEntities
