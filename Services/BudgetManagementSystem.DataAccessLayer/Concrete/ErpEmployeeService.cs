...
public class ErpEmployeeService(ErpDataDbContext context, StaffIDMaskDBContext staffIDMaskContext, CompetencyCoreDbContext competencydbcontext)
{
    ...
    public async Task<EmployeeData> GetEmployeeData(string employeeId)
    {
        var employee = await _context.EmployeeDetails.Select(e => new EmployeeData
        {
            UserName = e.UserName,
            EmailAddress = e.EmailAddress,
            FirstName = e.FirstName,
...
public List<OfficeJobRoleVm> GetJobRolesByOffice(StaffJobRoles officename)
    ...
    public async Task<List<String>> GetHeadOfOfficeIds(int empofficeid) => await _context.EmployeeDetails.AsNoTracking().Where(x => x.OfficeId == empofficeid && x.PersonTypeId == EmployeeType.ActiveStaff).Select(s=>s.HeadOfOfficeId).ToListAsync();

    public async Task<List<String>> GetHeadDivisionIds(int? empDivisionId) => await _context.EmployeeDetails.AsNoTracking().Where(x => x.DivisionId == empDivisionId && x.PersonTypeId == EmployeeType.ActiveStaff).Select(s => s.HeadOfDivId).ToListAsync();

    public async Task<List<String>> GetHeadDepartmentIds(int? empDeptId) => await _context.EmployeeDetails.AsNoTracking().Where(x => x.DepartmentId == empDeptId && x.PersonTypeId == EmployeeType.ActiveStaff).Select(s => s.HeadOfDeptId).ToListAsync();

    public async Task<List<String>> GetSubordinatesIds(int empofficeid) => await _context.EmployeeDetails.AsNoTracking().Where(x => x.OfficeId == empofficeid && x.PersonTypeId == EmployeeType.ActiveStaff).Select(s => s.HeadOfOfficeId).ToListAsync();

    public async Task<List<string>> GetGovernorDGsEmails() => await _context.EmployeeDetails.AsNoTracking().Where(x => x.JobName.Contains("GOVERNOR") && x.PersonTypeId == EmployeeType.ActiveStaff).Select(s => s.EmailAddress).ToListAsync();
